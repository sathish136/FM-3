...
const DEFAULT_ROLE_TEMPLATES: Omit<RoleTemplate, "id" | "createdAt" | "updatedAt">[] = [
  {
    ...
    color: "rose",
    moduleRoles: JSON.stringify(Object.fromEntries(ALL_MODULE_KEYS.map(k => [k, "write"]))),
  },
  {
    ...
    moduleRoles: JSON.stringify(Object.fromEntries([
      ...
      ...["purchase-order","purchase-orders","purchase-dashboard","stores-dashboard","logistics-dashboard","process-proposal",
         ...
         "plc-site-calls","plc-service-reports","plc-programs","plc-hmi-programs","plc-pid-design","plc-instruments","plc-tags","plc-panel-inspection","plc-support-tickets","plc-device-config","plc-network-architecture"].map(k => [k, "read"]),
      ...["task-management","team-pulse","team-reporting","ip-call-logs","ip-call-logs-hr","ip-call-logs-project","ip-call-logs-purchase",
         "ip-call-logs-marketing","meeting-discussion","speech-translator","translator","marketing","leads","campaigns","vc-card-scanner",
         "plant-enquiry","sales-dashboard","hrms-recruitment","hrms-incidents","hrms-work-monitor","site-data","plant-overview","site-db",
         "site-db-analyze","site-db-system","cctv","gallery","workshop","mis-report","payment-tracker","user-management","agent-management",
         "settings","email-settings","smart-inbox"].map(k => [k, "none"]),
    ])),
  },
  {
    name: "Engineer",
    description: "Full access to design & engineering and PLC/automation modules",
    color: "blue",
    moduleRoles: JSON.stringify(Object.fromEntries([
      ...["dashboard","calendar","tasks","design-3d","pid","nesting","project-drawings","email","chat",
         "plc-site-calls","plc-service-reports","plc-programs","plc-hmi-programs","plc-pid-design","plc-instruments","plc-tags","plc-panel-inspection","plc-support-tickets","plc-device-config","plc-network-architecture"].map(k => [k, "write"]),
      ...["projects","project-board","project-timeline","meeting-minutes","material-request","presentation","proposal-library","workshop"].map(k => [k, "read"]),
      ...["task-management","team-pulse","team-reporting","ip-call-logs","ip-call-logs-hr","ip-call-logs-project","ip-call-logs-purchase",
         "ip-call-logs-marketing","meeting-discussion","speech-translator","translator","purchase-order","purchase-orders","purchase-dashboard",
         "stores-dashboard","logistics-dashboard","process-proposal","finance-dashboard","marketing","leads","campaigns","vc-card-scanner",
         "plant-enquiry","sales-dashboard","hrms","hrms-checkin","hrms-leave-request","hrms-claims","hrms-recruitment","hrms-incidents",
         "hrms-analytics","hrms-performance","hrms-team-performance","hrms-task-summary","hrms-daily-reporting","hrms-work-monitor",
         "site-data","plant-overview","site-db","site-db-analyze","site-db-system","cctv","gallery","mis-report","payment-tracker",
         "user-management","agent-management","settings","email-settings","smart-inbox","sheets"].map(k => [k, "none"]),
    ])),
  },
  {
    name: "HR Manager",
    ...
    moduleRoles: JSON.stringify(Object.fromEntries([
      ...
      ...["task-management","team-pulse","team-reporting","ip-call-logs","ip-call-logs-hr","ip-call-logs-project","ip-call-logs-purchase",
         ...
         "design-3d","pid","nesting","plc-site-calls","plc-service-reports","plc-programs","plc-hmi-programs","plc-pid-design","plc-instruments",
         "plc-tags","plc-panel-inspection","plc-support-tickets","plc-device-config","plc-network-architecture","material-request","purchase-order","purchase-orders","purchase-dashboard","stores-dashboard",
         "logistics-dashboard","process-proposal","finance-dashboard","marketing","leads","campaigns","vc-card-scanner","plant-enquiry",
         "sales-dashboard","site-data","plant-overview","site-db","site-db-analyze","site-db-system","cctv","gallery","workshop","mis-report",
         "payment-tracker","user-management","agent-management","settings","email-settings","smart-inbox","sheets"].map(k => [k, "none"]),
    ])),
  },
  {
    name: "Finance",
    ...
    color: "amber",
    moduleRoles: JSON.stringify(Object.fromEntries([
      ...["dashboard","calendar","finance-dashboard","purchase-order","purchase-orders","purchase-dashboard","stores-dashboard",
         "logistics-dashboard","material-request","stock-reports","payment-tracker","email","chat"].map(k => [k, "write"]),
      ...["tasks","projects","project-board","process-proposal","mis-report"].map(k => [k, "read"]),
      ...["task-management","team-pulse","team-reporting","ip-call-logs","ip-call-logs-hr","ip-call-logs-project","ip-call-logs-purchase",
         "ip-call-logs-marketing","project-timeline","meeting-minutes","project-drawings","presentation","proposal-library","meeting-discussion",
         "speech-translator","translator","design-3d","pid","nesting","plc-site-calls","plc-service-reports","plc-programs","plc-hmi-programs",
         "plc-pid-design","plc-instruments","plc-tags","plc-panel-inspection","plc-support-tickets","plc-device-config","plc-network-architecture","marketing","leads","campaigns","vc-card-scanner","plant-enquiry",
         "sales-dashboard","hrms","hrms-checkin","hrms-leave-request","hrms-claims","hrms-recruitment","hrms-incidents","hrms-analytics",
         "hrms-performance","hrms-team-performance","hrms-task-summary","hrms-daily-reporting","hrms-work-monitor","site-data","plant-overview",
         "site-db","site-db-analyze","site-db-system","cctv","gallery","workshop","user-management","agent-management","settings",
         "email-settings","smart-inbox","sheets"].map(k => [k, "none"]),
    ])),
  },
  {
    name: "Viewer",
    ...
    moduleRoles: JSON.stringify(Object.fromEntries([
      ...
      ...ALL_MODULE_KEYS.filter(k => !["dashboard","calendar","tasks","projects","project-board","project-timeline","meeting-minutes"].includes(k)).map(k => [k, "none"]),
    ])),
  },
];
...
export function UserManagementContent() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [uRes, pRes, prRes, tRes, dRes] = await Promise.all([
        fetch(`${BASE}/api/erpnext-users?all=true`),
  ...
  useEffect(() => {
    ...
    const match = path.match(/\/app\/user\/([^/?#]+)/i);
    if (!match) return;
    const email = decodeURIComponent(match[1]).toLowerCase();
    (async () => {
      try {
        const res = await fetch(`${BASE}/api/erpnext-users/${encodeURIComponent(email)}`);
        if (!res.ok) return;
        const user: ErpUser = await res.json();
        setSelected(user);
      } catch {}
    })();
  }, []);
