...
import {
  Plus, Search, Loader2, Trash2, X, Monitor, ChevronDown,
  Upload, Download, File, FolderOpen, GitBranch, Info,
  CheckCircle2, Clock, AlertCircle, Layout as LayoutIcon, User,
...
const STATUS_CFG: Record<Status, { label: string; bg: string; text: string; border: string; dot: string; icon: any }> = {
  ...
  "Completed":   { label: "Completed",   bg: "bg-violet-50",   text: "text-violet-700",  border: "border-violet-300",  dot: "bg-violet-500",  icon: CheckCircle2 },
...
const EMPTY: HMIProgram = { version: "1.0", status: "Draft" };
...
export default function PLCHMIPrograms() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`${BASE}/api/plc/hmi-programs?${p}`);
  ...
  const save = async () => {
    if (!editing.program_no?.trim() && !editing.project_name?.trim()) {
      toast({ title: "Program No. or Project Name required", variant: "destructive" }); return;
    }
    setSaving(true);
    try {
      ...
      const url = isNew ? `${BASE}/api/plc/hmi-programs` : `${BASE}/api/plc/hmi-programs/${editing.id}`;
      const body = {
        ...editing,
      ...
      if (isNew) {
        ...
        setDrawerTab("backup");
        toast({ title: "HMI Program created — upload your backup files now" });
      } else {
        toast({ title: "HMI Program updated" });
        await load();
        const refreshed = await fetch(`${BASE}/api/plc/hmi-programs/${editing.id}`);
  ...
  const field = (k: keyof HMIProgram, v: any) => setEditing(p => ({ ...p, [k]: v }));
      ...
      <div className="flex flex-col h-full bg-[#f4f6fb]">
        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-violet-600 shadow shadow-violet-200">
              <Monitor className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="text-lg font-bold text-slate-900 leading-tight">HMI Programs</h1>
              <p className="text-xs text-slate-400 mt-0.5">Screen program registry · backup management · revision tracking</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search by program number, project…"
                className="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-400" />
            </div>
          ...
          {/* ── Table ─────────────────────────────────────────────────── */}
          {loading ? (
            <div className="flex justify-center py-20"><Loader2 className="w-7 h-7 animate-spin text-violet-500" /></div>
          ) : items.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-200 text-center py-24">
              <LayoutIcon className="w-12 h-12 mx-auto mb-3 text-slate-200" />
              <p className="font-semibold text-slate-500">No HMI programs found</p>
              <p className="text-sm text-slate-400 mt-1">Click "New HMI Program" to create the first entry</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {items.map(item => {
                    ...
                    return (
                      <tr key={item.id}
                        className="hover:bg-violet-50/50 transition-colors group cursor-pointer"
                        onClick={() => openEdit(item)}>
                        <td className="px-4 py-3">
                          <div className="font-semibold text-slate-800">{item.program_no || item.project_name || "—"}</div>
                          {item.program_no && item.project_name && (
                            <div className="text-xs text-slate-400 mt-0.5">{item.project_name}</div>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className="font-mono text-xs bg-violet-50 text-violet-700 border border-violet-100 px-2 py-0.5 rounded font-semibold">
                            v{item.version || "—"}
                          </span>
                          {revs.length > 0 && (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-violet-400 text-xs font-semibold uppercase tracking-widest">HMI Program</span>
                  </div>
                  <h2 className="text-white font-bold text-lg leading-tight truncate">
                    {editing.program_no || editing.project_name || (editing.id ? "Program Details" : "New HMI Program")}
                  </h2>
                ...
                <button key={tab.id} onClick={() => setDrawerTab(tab.id)}
                  ...
                  {tab.count !== null && tab.count > 0 && (
                    <span className={cn("ml-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold",
                      drawerTab === tab.id ? "bg-violet-100 text-violet-700" : "bg-slate-200 text-slate-600")}>
                      {tab.count}
                    </span>
                  )}
                </button>
                  ...
                  {!editing.id ? (
                    ...
                    <>
                      ...
                      <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
                        className="w-full group flex flex-col items-center justify-center gap-2 py-8 border-2 border-dashed border-violet-200 rounded-xl text-violet-600 hover:border-violet-400 hover:bg-violet-50 transition-all disabled:opacity-50">
                        {uploading
                          ? <Loader2 className="w-7 h-7 animate-spin" />
                          : <Upload className="w-7 h-7 group-hover:scale-110 transition-transform" />
                        }
                        <div className="text-center">
                          <p className="font-semibold text-sm">{uploading ? "Uploading…" : "Upload HMI Backup"}</p>
                          <p className="text-xs text-violet-400 mt-0.5">Click or drag · .ap15 · .hmi · .xte · any format · max 50 MB</p>
                        </div>
                      </button>
                      ...
                      {filesLoading ? (
                        ...
                        <div className="space-y-2">
                          ...
                          {files.map(f => (
                            <div key={f.id} className="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-xl group hover:border-violet-200 hover:bg-violet-50/30 transition-colors">
                ...
                <div className="p-6 space-y-5">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">
                        Program No. <span className="text-red-500">*</span>
                      </label>
                      <input value={editing.program_no ?? ""} onChange={e => field("program_no", e.target.value)}
                        placeholder="HMI-001"
                        className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 bg-slate-50 focus:bg-white transition-colors" />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea value={editing.notes ?? ""} onChange={e => field("notes", e.target.value)} rows={2}
                      ...
                      className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 bg-slate-50 focus:bg-white transition-colors resize-none" />
                  </div>
                      ...
                      {[...(editing.revisions ?? [])].reverse().map((rev, i, arr) => (
                        <div key={i} className="relative pl-8 pb-6 last:pb-0">
                          ...
                          <div className={cn(
                            ...
                            i === 0 ? "bg-violet-600 border-violet-600" : "bg-white border-slate-300"
                          )}>
            ...
            <div className="border-t border-slate-200 px-6 py-4 bg-slate-50 flex items-center justify-between flex-shrink-0">
              ...
              {drawerTab === "info" && (
                <button onClick={save} disabled={saving}
                  className="flex items-center gap-2 px-5 py-2 bg-violet-600 text-white text-sm rounded-lg hover:bg-violet-700 disabled:opacity-60 transition-colors font-semibold shadow-sm">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4 border border-slate-100">
            ...
            <h3 className="font-bold text-slate-800 text-center mb-1">Delete HMI Program?</h3>
