...
import {
  ...
  Search, RefreshCw, ExternalLink, Loader2,
} from "lucide-react";
...
function AttachLink({ url }: { url: string | null }) {
  if (!url) return <span className="text-xs text-gray-300">—</span>;
  const href = url.startsWith("http") ? url : `https://erp.wttint.com${url}`;
  return (
    <a
      href={href}
      target="_blank"
      rel="noreferrer"
      className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-blue-600 hover:bg-blue-50 border border-blue-200 transition-colors"
    >
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-100">
              {filtered.map((d) => (
                <div
                  key={d.name}
                  className="grid grid-cols-[2fr_1fr_130px_110px_90px] gap-4 px-5 py-3.5 items-center hover:bg-gray-50 transition-colors"
                >
