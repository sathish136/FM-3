...
export default function CostWorking() {
  ...
  const [itemLookupResults, setItemLookupResults] = useState<ErpItemMatch[]>([]);
  const [itemLookupLoading, setItemLookupLoading] = useState(false);
  const [selectedLookupItems, setSelectedLookupItems] = useState<Set<number>>(new Set());
  const [addingLookupItems, setAddingLookupItems] = useState(false);
  ...
  async function uploadStep(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Auto-lookup ERP items based on filename
      const rawName = file.name.replace(/\.(step|stp)$/i, "").replace(/[_\-]+/g, " ").trim();
      openItemLookup(rawName);
    } catch (e: any) {
      toast({ title: e.message, variant: "destructive" });
    } finally { setUploading(false); if (fileRef.current) fileRef.current.value = ""; }
  }

  async function openItemLookup(query: string) {
    setItemLookupQuery(query);
    setItemLookupResults([]);
    setSelectedLookupItems(new Set());
    setItemLookupModal(true);
    setItemLookupLoading(true);
    try {
      const r = await fetch(`${BASE}/api/cost-working/erp/item-lookup?q=${encodeURIComponent(query)}`);
      const d = await r.json();
      setItemLookupResults(d.items ?? []);
    } finally { setItemLookupLoading(false); }
  }

  async function searchItemLookup(q: string) {
    setItemLookupQuery(q);
    if (!q.trim()) { setItemLookupResults([]); return; }
    setItemLookupLoading(true);
    try {
      const r = await fetch(`${BASE}/api/cost-working/erp/item-lookup?q=${encodeURIComponent(q)}`);
      const d = await r.json();
      setItemLookupResults(d.items ?? []);
    } finally { setItemLookupLoading(false); }
  }

  async function addLookupItems() {
    if (!activeSession || selectedLookupItems.size === 0) return;
    setAddingLookupItems(true);
    try {
      const toAdd = itemLookupResults.filter((_, i) => selectedLookupItems.has(i));
      for (const it of toAdd) {
        await fetch(`${BASE}/api/cost-working/sessions/${activeSession.id}/items`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      await loadItems(activeSession.id);
      await loadSessions();
      setItemLookupModal(false);
      toast({ title: `${toAdd.length} item(s) added from ERP` });
    } finally { setAddingLookupItems(false); }
  }
  ...
  return (
    <Layout>
      ...
      {/* ── ERP Item Lookup Modal (post-upload) ──────────────────────────── */}
      {itemLookupModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 shrink-0">
              <div>
                <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2">
                  ...
                  ERP Item Price Lookup
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">Matched ERP items from the 3D part name — select to add with latest PO price</p>
              </div>
              <button onClick={() => setItemLookupModal(false)} className="text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="p-4 border-b border-slate-100 shrink-0">
              <div className="relative">
                ...
                <input
                  ...
                  onChange={e => searchItemLookup(e.target.value)}
                  placeholder="Search ERP items by name…"
                  className="w-full pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {itemLookupLoading ? (
                <div className="flex flex-col items-center justify-center py-10 gap-2 text-slate-400">
                  ...
                  <p className="text-xs">Searching ERP items…</p>
                </div>
              ) : itemLookupResults.length === 0 ? (
                <div className="text-center py-10 text-xs text-slate-400">
                  <Package className="w-8 h-8 mx-auto mb-2 opacity-20" />
                  {itemLookupQuery ? `No ERP items found matching "${itemLookupQuery}"` : "Type to search ERP items"}
                </div>
              ) : (
                <>
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs text-slate-500">{itemLookupResults.length} item(s) found in ERP</p>
                    <button
                      onClick={() => setSelectedLookupItems(
                        selectedLookupItems.size === itemLookupResults.length
                          ? new Set()
                          : new Set(itemLookupResults.map((_, i) => i))
                      )}
                      className="text-xs text-indigo-500 hover:underline">
                      {selectedLookupItems.size === itemLookupResults.length ? "Deselect all" : "Select all"}
                    </button>
                  </div>
                  <div className="space-y-1.5">
                    {itemLookupResults.map((it, i) => (
                      <label key={i}
                        className={cn("flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors",
                          selectedLookupItems.has(i) ? "bg-indigo-50 border-indigo-200" : "bg-slate-50 border-slate-100 hover:bg-white")}>
                        <input type="checkbox" checked={selectedLookupItems.has(i)}
                          onChange={() => {
                            const n = new Set(selectedLookupItems);
                            n.has(i) ? n.delete(i) : n.add(i);
                            setSelectedLookupItems(n);
                          }}
                          className="mt-0.5 accent-indigo-600" />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-slate-800 truncate">{it.item_name}</p>
                          <p className="text-[10px] text-slate-400 truncate">{it.item_code}{it.description ? ` — ${it.description}` : ""}</p>
                          {it.latest_po_no && (
                            <span className="inline-flex items-center gap-0.5 text-[9px] bg-indigo-50 text-indigo-500 border border-indigo-100 px-1.5 rounded mt-0.5">
                              PO: {it.latest_po_no}
                            </span>
                          )}
                        </div>
                        <div className="text-right shrink-0">
                          {it.latest_po_rate != null ? (
                            <>
                              <p className="text-xs font-semibold text-emerald-700">{fmtCur(it.latest_po_rate)}</p>
                              <p className="text-[10px] text-slate-400">per {it.latest_uom || "Nos"} · latest PO</p>
                            </>
                          ) : (
                            <p className="text-[10px] text-amber-500 flex items-center gap-1">
                              <AlertCircle className="w-3 h-3" /> No PO price
                            </p>
                          )}
                        </div>
                      </label>
            ...
            <div className="flex items-center justify-between p-4 border-t border-slate-100 shrink-0">
              <span className="text-xs text-slate-400">{selectedLookupItems.size} item(s) selected</span>
              <div className="flex gap-2">
                <button onClick={() => setItemLookupModal(false)} className="px-4 py-2 text-xs text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Skip</button>
                <button onClick={addLookupItems} disabled={selectedLookupItems.size === 0 || addingLookupItems}
                  className="px-4 py-2 text-xs bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-1.5">
                  {addingLookupItems && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                  Add {selectedLookupItems.size > 0 ? `(${selectedLookupItems.size})` : ""} to Cost Sheet
                </button>
              </div>
            </div>
          </div>
