...
import {
  ...
  Move3d, ScanLine, Info, Tag, Crosshair, ArrowUpDown,
  ZoomIn, ZoomOut, Eye,
} from "lucide-react";
...
// ─── Pan/Zoom canvas for fabrication sheet ────────────────────────────────────
function SheetCanvas({ children }: { children: React.ReactNode }) {
  ...
  const onWheel = (e: React.WheelEvent) => {
    e.preventDefault();
    setZoom(z => Math.min(2.5, Math.max(0.25, z * (e.deltaY < 0 ? 1.08 : 0.93))));
  };

  return (
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
      {/* Toolbar */}
      <div className="shrink-0 bg-muted/40 border-b border-border px-3 py-1 flex items-center gap-2">
        <span className="text-[10px] text-muted-foreground">Drag to pan · Scroll to zoom</span>
        ...
        <button type="button" onClick={() => setZoom(z => Math.max(0.25, z * 0.87))}
          className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors">
          <ZoomOut className="w-3.5 h-3.5" />
        </button>
        <button type="button" onClick={() => { setZoom(0.82); setPan({ x: 24, y: 16 }); }}
          className="px-2 py-0.5 rounded text-[10px] bg-background border border-border hover:border-sky-400 hover:text-sky-600 transition-colors">
          Reset
        </button>
      </div>
      ...
      {/* Canvas */}
      <div
        className="flex-1 min-h-0 overflow-hidden relative bg-slate-300/40 dark:bg-slate-900/60"
