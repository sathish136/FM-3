...
function Panel({
  ...
  const filtered = useFilter(rows, s, cols.map((c) => c.key));
  const PAGE = 50;
  const [visible, setVisible] = useState(PAGE);
  // reset paging when search changes
  useMemo(() => { setVisible(PAGE); }, [s]);
  const pageRows = filtered.slice(0, visible);

  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
      <div className={cn("flex items-center gap-2 px-4 py-2.5 border-b border-gray-100", m.tint)}>
        <Icon className={cn("w-3.5 h-3.5 shrink-0", m.text)} />
        <span className={cn("font-bold text-xs uppercase tracking-wide", m.text)}>{m.label}</span>
        <span className={cn("ml-1 text-[10px] font-bold text-white px-1.5 py-0.5 rounded-full", m.dot)}>
          {q.isLoading ? "…" : total}
        </span>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search"
            className="pl-6 pr-2 py-1 text-[10px] rounded-md border border-gray-200 bg-white w-32 focus:outline-none focus:border-gray-400"
          />
        </div>
        <button
          onClick={() => q.refetch()}
        ...
        {q.isLoading ? (
          ...
          <table className="w-full text-xs border-separate border-spacing-0">
            <thead className="sticky top-0 z-10">
              <tr>
                <th className={cn("px-3 py-2.5 text-left font-extrabold uppercase text-[9px] tracking-widest w-10 border-b backdrop-blur-sm", m.tint, m.border, m.text)}>#</th>
                {cols.map((c) => (
                  <th key={c.key} className={cn("px-3 py-2.5 text-left font-extrabold uppercase text-[9px] tracking-widest whitespace-nowrap border-b backdrop-blur-sm", m.tint, m.border, m.text)}>
                    {c.label}
                  </th>
                ))}
              </tr>
              ...
              {pageRows.map((r, i) => (
                ...
                >
                  <td className="px-3 py-2 border-b border-gray-100">
                    <span className={cn(
                      "inline-flex items-center justify-center w-5 h-5 rounded-md text-[9px] font-bold tabular-nums",
                      "bg-gray-100 text-gray-500 group-hover:bg-indigo-100 group-hover:text-indigo-700 transition-colors"
                    )}>{i + 1}</span>
                  </td>
                  {cols.map((c) => (
                    <td key={c.key} className="px-3 py-2 border-b border-gray-100 text-[11px] text-gray-700 max-w-[280px]">
                      <div className="truncate" title={String(r[c.key] ?? "")}>
                        {renderCell ? renderCell(c.key, r) : smartCell(c.key, r)}
                      </div>
                    </td>
                  ))}
                </tr>
      ...
      <div className="p-4 space-y-3 max-w-[1800px] mx-auto bg-gray-50/40 min-h-full">
        ...
        {tab === "Overview" && (
          <>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
              <Panel k="open_leads"     cols={OPEN_LEAD_COLS} height={300} />
        ...
        {/* ── OPEN LEADS TAB ── */}
        {tab === "Open Leads" && (
          <Panel k="open_leads" cols={OPEN_LEAD_COLS} height={680} />
        )}
