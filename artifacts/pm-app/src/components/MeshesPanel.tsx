...
export default function MeshesPanel({
  ...
  const [expanded, setExpanded] = useState<Set<string>>(() => new Set(["root-0"]));
  ...
  const toggleAllVisibility = () => {
    if (allHidden) {
      onToggleMesh(allIndices, false);
    } else {
      onToggleMesh(allIndices, true);
    }
  };
  ...
  return (
    <div className="w-56 bg-[#13131f] border-l border-white/10 flex flex-col flex-shrink-0 h-full">
      <div className="px-3 pt-3 pb-2 border-b border-white/10 flex-shrink-0">
        <div className="text-xs font-semibold text-gray-200 mb-2">Parts</div>
