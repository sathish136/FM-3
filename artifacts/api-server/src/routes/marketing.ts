...
// GET /api/marketing/leads-slim  → just { name, company_name } for proposal-library matching
router.get("/marketing/leads-slim", async (_req, res) => {
  const url = `${ERP_BASE}/api/method/wtt_module.customization.custom.rfq.get_all_lead_details`;
  try {
    const data = await erpFetch(url);
    const leads: any[] = data?.message ?? [];
    const slim = leads.map((l: any) => ({
      name: l.name ?? "",
      company_name: l.company_name ?? "",
      country: l.country ?? "",
      status: l.status ?? "",
    }));
    return res.json({ leads: slim });
  } catch (err: any) {
    console.error("marketing/leads-slim error:", err.message);
    return res.json({ leads: [] });
  }
});
