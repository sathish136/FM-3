...
import { classifyPlant, plantBadgeColor, type PlantClassification } from "../lib/plantTypeDetect";
...
// ───────────────────────────────────────────────────────────
// GET /api/site-db/analytics/system-overview?refresh=1
// "All databases, system-wise" — for every visible plant DB returns the
// list of every time-series-like table together with its detected plant
// type, row count, column count and last-update timestamp. Powers the
// new System Overview page that links one-click into the deep dashboard.
// Aggressively cached (5 min) because it touches every DB on the box.
// ───────────────────────────────────────────────────────────
type SystemTable = {
  schema: string;
  table: string;
  ...
  ageMin: number | null;
  classification: PlantClassification & { badgeColor: string };
};
...
type SystemPlant = {
  ...
  tableCount: number;
  totalRows: number;
  primaryType: PlantClassification["type"];
  primaryLabel: string;
...
let systemOverviewCache: { at: number; data: any } | null = null;
const SYS_OVERVIEW_TTL_MS = 5 * 60 * 1000;

router.get("/site-db/analytics/system-overview", async (req, res) => {
  const force = req.query.refresh === "1";
  if (!force && systemOverviewCache && Date.now() - systemOverviewCache.at < SYS_OVERVIEW_TTL_MS) {
    return res.json({
      ...systemOverviewCache.data,
      cached: true,
      ageSec: Math.round((Date.now() - systemOverviewCache.at) / 1000),
    });
  }
  ...
  try {
    ...
    const plants: SystemPlant[] = await Promise.all(allDbs.map(async (dbName: string) => {
      try {
        const pool = await getPool(dbName);

        // Pull all tables that look like data tables (>=100 rows) and ALL their columns
        // in two queries — much cheaper than per-table round-trips.
        const tabR = await pool.request().query(`
          SELECT s.name AS schemaName, t.name AS tableName, SUM(p.rows) AS row_total
          FROM sys.tables t
          INNER JOIN sys.schemas s ON s.schema_id = t.schema_id
          ...
          ORDER BY SUM(p.rows) DESC
        `);
        // Cap at 12 to stay snappy — anything below that is unlikely to be a SCADA table.
        const candidates: { schemaName: string; tableName: string; row_total: number }[] =
          (tabR.recordset || []).slice(0, 12);
        if (candidates.length === 0) {
          return {
            db: dbName, tableCount: 0, totalRows: 0,
            primaryType: "generic", primaryLabel: "Process Plant",
            primaryBadgeColor: "slate", tables: [],
          };
        }
        ...
        const tableResults = await Promise.all(candidates.map(async (tab): Promise<SystemTable | null> => {
          const cols = colsByTable.get(`${tab.schemaName}.${tab.tableName}`) || [];
          const timeCol = cols.find(c => DATE_TYPES.has(c.type))?.name || null;
          const numericCols = cols.filter(c => NUMERIC_TYPES.has(c.type)).map(c => c.name);
          // Keep only candidates with a time column AND at least a couple of numeric tags —
          // anything else isn't a SCADA time-series table worth dashboarding.
          if (!timeCol || numericCols.length < 2) return null;

          const cls = classifyPlant(cols.map(c => c.name));

          let lastUpdateIso: string | null = null;
          let ageMin: number | null = null;

          if (safeIdent(tab.schemaName) && safeIdent(tab.tableName) && safeIdent(timeCol)) {
            try {
              const req2 = pool.request();
              (req2 as any).timeout = 8000;
              const lastR = await req2.query(`
                SELECT TOP 1 [${timeCol}] AS __time
                FROM [${tab.schemaName}].[${tab.tableName}] WITH (NOLOCK)
                ORDER BY [${timeCol}] DESC
              `);
              const t: Date | null = lastR.recordset?.[0]?.__time
                ? new Date(lastR.recordset[0].__time) : null;
              if (t) {
                lastUpdateIso = t.toISOString();
                ageMin = Math.round((Date.now() - t.getTime()) / 60000);
                if (ageMin < 0) ageMin = 0;
              }
            } catch {
              // ignore — table classification still useful without a freshness reading
            }
          }
          ...
          return {
            ...
            ageMin,
            classification: { ...cls, badgeColor: plantBadgeColor(cls.type) },
          };
        }));

        const tables = tableResults.filter((t): t is SystemTable => !!t);

        // Pick the "primary" plant type for the DB = the type with most tables,
        // ties broken by which one has a non-zero unitCount (more confident detection).
        const typeCounts: Record<string, { type: PlantClassification["type"]; label: string; badgeColor: string; n: number; conf: number }> = {};
        for (const t of tables) {
          const key = t.classification.type;
          if (!typeCounts[key]) {
            typeCounts[key] = {
              type: t.classification.type,
              label: t.classification.label,
              badgeColor: t.classification.badgeColor,
              n: 0,
              conf: 0,
            };
          }
          typeCounts[key].n += 1;
          if (t.classification.unitCount > 0) typeCounts[key].conf += 1;
        }
        const ranked = Object.values(typeCounts)
          .sort((a, b) => (b.n - a.n) || (b.conf - a.conf));
        const primary = ranked[0] || {
          type: "generic" as const, label: "Process Plant", badgeColor: "slate",
        };
        ...
        return {
          ...
          primaryBadgeColor: primary.badgeColor,
          tables,
        };
      } catch (e: any) {
        return {
          db: dbName, tableCount: 0, totalRows: 0,
          primaryType: "generic" as const, primaryLabel: "Process Plant",
          primaryBadgeColor: "slate", tables: [],
    ...
    // Aggregate counts by detected type for the page hero strip
    const typeTotals: Record<string, { type: string; label: string; badgeColor: string; dbs: number; tables: number }> = {};
    let totalTables = 0;
    let totalRows = 0;
    for (const p of plants) {
      totalTables += p.tableCount;
      totalRows += p.totalRows;
      const key = p.primaryType;
      if (!typeTotals[key]) {
        typeTotals[key] = {
          type: p.primaryType,
          label: p.primaryLabel,
          badgeColor: p.primaryBadgeColor,
          dbs: 0, tables: 0,
        };
      }
      typeTotals[key].dbs += 1;
      typeTotals[key].tables += p.tableCount;
    }

    const data = {
      generatedAt: new Date().toISOString(),
      totals: {
        databases: plants.length,
        tables: totalTables,
        rows: totalRows,
        byType: Object.values(typeTotals).sort((a, b) => b.dbs - a.dbs),
      },
      plants: plants.sort((a, b) => a.db.localeCompare(b.db)),
    };
    systemOverviewCache = { at: Date.now(), data };
    return res.json({ ...data, cached: false });
  } catch (e: any) {
    return res.status(502).json({ error: e.message });
  }
