...
// Some ERPNext fields (e.g. `actual_expense`) are blocked by Frappe's query
// permissions and cause a 417 "Field not permitted in query" error. This
// helper fetches a Frappe `resource` list, and if such an error comes back it
// transparently retries with the offending field(s) removed so the rest of the
// data still loads. Returns `{ data, droppedFields }`.
async function fetchErpResourceList(
  doctype: string,
  fields: string[],
  extraParams: Record<string, string> = {},
): Promise<{ data: any[]; droppedFields: string[] }> {
  ...
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    if (res.ok) {
      ...
      return { data: (json.data || []) as any[], droppedFields };
    }
    ...
    if (res.status === 417) {
      ...
      if (match) {
        ...
        if (currentFields.includes(blocked)) {
          droppedFields.push(blocked);
          currentFields = currentFields.filter(f => f !== blocked);
          continue;
        }
      }
  ...
  throw new Error(`ERPNext API: too many blocked fields while fetching ${doctype}`);
}
...
// Upload an arbitrary file to ERPNext's File doctype. Returns the file_url
// (e.g. "/private/files/foo.step") which can then be assigned to a doctype's
// attach field. If `attachedTo` is provided, the file is also linked to that
// document in ERPNext.
export async function uploadErpNextFile(
  buffer: Buffer,
  fileName: string,
  options?: {
    isPrivate?: boolean;
  ...
  const blob = new Blob([buffer], { type: options?.contentType || "application/octet-stream" });
  const form = new FormData();
  form.append("file", blob, fileName);
  form.append("is_private", options?.isPrivate === false ? "0" : "1");
  form.append("folder", options?.folder || "Home");
  if (options?.attachedToDoctype) form.append("doctype", options.attachedToDoctype);
  if (options?.attachedToName) form.append("docname", options.attachedToName);
  if (options?.attachedToField) form.append("fieldname", options.attachedToField);

  const res = await fetch(`${ERPNEXT_URL}/api/method/upload_file`, {
    method: "POST",
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error(`ERPNext upload_file error ${res.status}: ${body}`);
  }

  const json = await res.json() as any;
  const fileUrl: string = json?.message?.file_url || "";
  const name: string = json?.message?.name || "";
  if (!fileUrl) throw new Error("ERPNext upload_file returned no file_url");
  return { fileUrl, name };
}

export async function createErpNextDesign3D(payload: {
  project?: string;
  department?: string;
  tag?: string;
  system_name?: string;
  ...
  const url = `${ERPNEXT_URL}/api/resource/Design 3D`;
  const res = await fetch(url, {
    method: "POST",
    headers: { Authorization: authHeader(), "Content-Type": "application/json" },
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error(`ERPNext create Design 3D error ${res.status}: ${body}`);
  }
  const json = await res.json();
...
export async function fetchErpNextProjects(): Promise<ErpProject[]> {
  const fields = [
    ...
    "percent_complete", "expected_start_date", "expected_end_date", "notes",
    "creation", "estimated_costing", "actual_expense", "department",
  ];

  const filters = JSON.stringify([["Project", "status", "=", "On going"]]);

  const { data } = await fetchErpResourceList("Project", fields, {
    filters,
    limit_page_length: "500",
    order_by: "modified desc",
  });
  ...
  return data.map((p: any) => ({
    ...
    // Extra fields for richer dashboards (Meeting Discussion etc.)
    expectedStartDate: p.expected_start_date || null,
    expectedEndDate: p.expected_end_date || null,
    estimatedCosting: Number(p.estimated_costing) || 0,
...
export async function fetchErpNextProjectList(): Promise<ErpProjectListItem[]> {
  ...
  let data: any[];
  try {
    ({ data } = await fetchErpResourceList("Project", fields, {
...
export async function fetchErpNextLeaveApplications(filters?: { status?: string; employee?: string }): Promise<ErpLeaveApplication[]> {
  ...
  const fields = JSON.stringify([
    ...
    "month", "year", "proceed_to_take_leave", "approved_by",
  ]);
  const fArr: any[] = [];
  if (filters?.status === "Approved")  fArr.push(["Leave Request", "proceed_to_take_leave", "=", 1]);
  if (filters?.status === "Pending")   fArr.push(["Leave Request", "proceed_to_take_leave", "=", 0]);
  if (filters?.employee) fArr.push(["Leave Request", "employee", "=", filters.employee]);
  const params = new URLSearchParams({ fields, limit_page_length: "200", order_by: "modified desc" });
  if (fArr.length) params.set("filters", JSON.stringify(fArr));
  const url = `${ERPNEXT_URL}/api/resource/Leave Request?${params}`;
  const res = await fetch(url, { headers: { Authorization: authHeader() } });
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error(`ERPNext leave request: ${res.status} — ${body}`);
  }
  const data = await res.json();
  return data.data ?? [];
}
...
export async function fetchErpNextCheckins(filters?: {
  ...
  const extra: Record<string, string> = { limit_page_length: "500", order_by: "time asc" };
  if (fArr.length) extra.filters = JSON.stringify(fArr);
  try {
    const { data } = await fetchErpResourceList("Employee Checkin", fields, extra);
    return data as ErpEmployeeCheckin[];
  } catch (e: any) {
    throw new Error(`ERPNext checkin: ${e?.message || e}`);
  }
}
...
export async function createErpNextLeaveApplication(payload: {
  ...
  const res = await fetch(`${ERPNEXT_URL}/api/resource/Leave Request`, {
    method: "POST",
    headers: { Authorization: authHeader(), "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
...
export async function fetchErpNextExpenseClaims(filters?: {
  ...
  const fields = JSON.stringify([
    ...
    "grand_total", "remark", "is_paid", "department",
  ]);
  const fArr: any[] = [];
  if (filters?.employee)                     fArr.push(["Claim Request", "employee", "=", filters.employee]);
  if (filters?.approval_status === "Paid")   fArr.push(["Claim Request", "is_paid", "=", 1]);
  if (filters?.approval_status === "Unpaid") fArr.push(["Claim Request", "is_paid", "=", 0]);
  const params = new URLSearchParams({ fields, limit_page_length: "200", order_by: "modified desc" });
  if (fArr.length) params.set("filters", JSON.stringify(fArr));
  const url = `${ERPNEXT_URL}/api/resource/Claim Request?${params}`;
  const res = await fetch(url, { headers: { Authorization: authHeader() } });
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new Error(`ERPNext claim request: ${res.status} — ${body}`);
  }
  const data = await res.json();
  return data.data ?? [];
}
...
export async function createErpNextExpenseClaim(payload: {
  ...
  if (!ERPNEXT_URL) throw new Error("ERPNext not configured");
  const body = { ...payload, docstatus: 0 };
  const res = await fetch(`${ERPNEXT_URL}/api/resource/Claim Request`, {
    method: "POST",
    headers: { Authorization: authHeader(), "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`ERPNext create claim request: ${res.status} — ${err}`);
  }
  const json = await res.json();
  return { name: json.data?.name || "" };
}

export async function fetchErpNextUsers(opts?: { skipFilter?: boolean }): Promise<ErpUser[]> {
  if (!ERPNEXT_URL) throw new Error("ERPNext not configured");

  const blockedUserIds = new Set<string>();

  if (!opts?.skipFilter) {
    // Build a set of user emails linked to blocked employees (HRMS views only)
    try {
  ...
  const data = await res.json();
  return (data.data ?? [])
    .filter((u: any) => opts?.skipFilter || USER_EMAIL_ALLOWLIST.has(u.name) || !blockedUserIds.has(u.name))
    .map((u: any) => ({
      email: u.name,
...
export async function fetchErpNextUser(email: string): Promise<ErpUser | null> {
  if (!ERPNEXT_URL) throw new Error("ERPNext not configured");
  const normalized = email.trim().toLowerCase();
  if (!normalized) return null;
  const url = `${ERPNEXT_URL}/api/resource/User/${encodeURIComponent(normalized)}?fields=${encodeURIComponent(JSON.stringify(["name", "full_name", "user_image", "enabled"]))}`;
  const res = await fetch(url, { headers: { Authorization: authHeader() } });
  if (!res.ok) return null;
  const data = await res.json();
  const u = data.data;
