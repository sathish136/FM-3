...
export const projectDrawingsTable = pgTable("project_drawings", {
  ...
  workflowStatus: text("workflow_status").notNull().default("pending_review"),
  reviewerEmail: text("reviewer_email"),
  uploaderEmail: text("uploader_email"),
  corrections: jsonb("corrections"),
  hodRemarks: text("hod_remarks"),
  assignedTeams: jsonb("assigned_teams").default([]),
  issueTriggerID: text("issue_trigger_id"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const design3dRecordsTable = pgTable("design_3d_records", {
  ...
  // Local-upload fields (when source = "local"). For ERPNext-sourced rows these are null.
  source: text("source").notNull().default("erpnext"),
