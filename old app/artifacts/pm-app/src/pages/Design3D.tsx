...
import {
  ...
  Eye, EyeOff, ChevronDown, FilterX, Plus, Upload, CheckCircle2,
} from "lucide-react";
...
function CreateDesign3DModal({
  ...
  const reset = () => {
    setProject(""); setDepartment(""); setTag(""); setSystemName("");
    setRevision(""); setFile(null); setSubmitting(false); setProgress(0);
    setErrMsg(null); setSuccess(false); setDragOver(false);
  };

  useEffect(() => { if (!open) reset(); }, [open]);

  const validExt = (name: string) =>
    /\.(step|stp|iges|igs)$/i.test(name);

  const pickFile = (f: File | null) => {
    if (!f) return;
    if (!validExt(f.name)) {
      setErrMsg("Only .step, .stp, .iges, .igs files are supported");
      return;
    }
    if (f.size > 500 * 1024 * 1024) {
      setErrMsg("File is larger than 500 MB");
      return;
    }
    setErrMsg(null);
    setFile(f);
  ...
  const submit = async () => {
    if (!file) { setErrMsg("Please choose a STEP/IGES file"); return; }
    setSubmitting(true);
    setErrMsg(null);
    setProgress(0);
    ...
    if (systemName) fd.append("system_name", systemName);
    if (revision) fd.append("revision", revision);

    try {
      // Use XHR for upload progress reporting.
      // Progress events fire many times per second — throttle UI updates to once
      // per animation frame so React doesn't re-render the modal dozens of times
      // per second (which makes the page feel frozen during large uploads).
      const result: any = await new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", `${BASE}/api/design-3d`);
        ...
        xhr.upload.onprogress = (e) => {
          ...
          pendingPct = Math.round((e.loaded / e.total) * 100);
          if (!rafScheduled) {
            rafScheduled = true;
      ...
      setSuccess(true);
      setProgress(100);
      // Brief success flash, then close + refresh
      setTimeout(() => {
        onCreated();
        onClose();
      }, 900);
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-gray-900">New 3D Design</h2>
              <p className="text-xs text-gray-500">Upload a STEP/IGES file to ERPNext</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
            ...
            <div
              ...
              className={`relative border-2 border-dashed rounded-xl px-4 py-6 text-center cursor-pointer transition-colors ${
                dragOver
                  ...
                  : "border-gray-300 bg-gray-50 hover:border-purple-300 hover:bg-purple-50/40"
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {file ? (
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-xs text-gray-500">{fileSizeMB} MB · click to change</span>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-1.5 text-gray-500">
                  <Upload className="w-6 h-6 text-gray-400" />
                  <span className="text-sm font-medium text-gray-700">Click to upload or drag & drop</span>
                  <span className="text-xs text-gray-400">.step · .stp · .iges · .igs (up to 500 MB)</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              disabled={submitting}
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:opacity-50"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                disabled={submitting}
                placeholder="e.g. Brine Loop"
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:opacity-50"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1.5">Revision</label>
              <input
                value={revision}
          ...
          {submitting && (
            <div>
              <div className="flex items-center justify-between text-xs text-gray-500 mb-1.5">
                <span>{success ? "Saved" : progress < 100 ? "Uploading…" : "Creating record…"}</span>
                <span>{progress}%</span>
              </div>
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all duration-200 ${success ? "bg-green-500" : "bg-purple-500"}`}
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-3.5 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
          <button
            onClick={onClose}
            disabled={submitting}
            className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-white disabled:opacity-50 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-colors"
          >
            {success ? (
  ...
  const loadProjects = useCallback(async () => {
    try {
      ...
      const list: ProjectOption[] = Array.isArray(data)
        ? data.map((p: any) => ({ name: p.name, project_name: p.project_name }))
        : [];
      setProjectOptions(list);
    } catch {
      // Non-blocking — Project field will just be empty
    }
  }, []);
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-sm font-medium text-white shadow-sm transition-colors"
              >
