  ...
  return (
    <div className="bg-slate-50 min-h-full">
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
        ...
        {!loading && !err && (
          <>
            <ExecutiveSummary insights={insights} period={period} plant={plant} />
            ...
            />
            <ProductionSection insights={insights} dailyRows={dailyRows} plant={plant} overallRecoveryRows={opsRows} />
            <StagePressureSection insights={insights} opsRows={opsRows} plant={plant} />
            <RecoveryBreakdownSection insights={insights} opsRows={opsRows} plant={plant} />
            ...
            <Recommendations insights={insights} plant={plant} />
          </>
        )}
      </div>
    </div>
...
type SkidInsight = {
  ...
  // RO stage extensions (optional; null/0 when columns are absent)
  dp: { avg: number | null; max: number | null; min: number | null; trend: number | null; hoursWarn: number; hoursCrit: number };
  inP: { avg: number | null; max: number | null };
  outP: { avg: number | null; max: number | null };
  recovery: { avg: number | null; min: number | null; max: number | null };
...
function buildInsights(plant: PlantSchema, opsRows: SeriesRow[], dailyRows: SeriesRow[], period: Period): Insights {
  ...
  const skidInsights: SkidInsight[] = plant.skids.map(s => {
    ...
    const dpS = colSeries(s.dp);
    const inPS = colSeries(s.inPressure);
    const outPS = colSeries(s.outPressure);
    const recS = colSeries(s.recovery);
    ...
    // DP behaves analogously to TMP for RO stages: rising DP across a stage
    // signals fouling / scaling. Thresholds tuned to typical multi-stage RO.
    const dpAvg = avg(dpS);
    const dpMax = maxOf(dpS);
    const dpMin = minOf(dpS);
    const dpHoursWarn = countWhere(dpS, v => v > 1.0 && v <= 1.5) * hoursPerBucket;
    const dpHoursCrit = countWhere(dpS, v => v > 1.5) * hoursPerBucket;
    const dpSlopePerBucket = slope(dpS);
    const dpTrend = dpSlopePerBucket != null ? dpSlopePerBucket * (24 / hoursPerBucket) : null;
    ...
    if (tmpHoursCrit > 0 || dpHoursCrit > 0 || lvlOOS > 0) status = "critical";
    else if (tmpHoursWarn > 0 || dpHoursWarn > 0) status = "warn";
    ...
    return {
      ...
      dp: { avg: dpAvg, max: dpMax, min: dpMin, trend: dpTrend, hoursWarn: dpHoursWarn, hoursCrit: dpHoursCrit },
      inP: { avg: avg(inPS), max: maxOf(inPS) },
      outP: { avg: avg(outPS), max: maxOf(outPS) },
      recovery: { avg: avg(recS), min: minOf(recS), max: maxOf(recS) },
...
// ── Executive summary ──────────────────────────────────────────────────
function ExecutiveSummary({ insights, period, plant }: { insights: Insights; period: Period; plant: PlantSchema }) {
  const lines: { tone: "good" | "warn" | "bad" | "info"; text: string }[] = [];
  const isRo = plant.type === "ro" || plant.type === "ro_reject";
  const unit = plant.unitLabel;
  const recoveryTarget = plant.type === "ro_reject" ? 50 : (plant.type === "ro" ? 75 : 90);
  const recoveryTargetLabel = plant.type === "ro_reject" ? "50%+" : (plant.type === "ro" ? "75%+" : "90%+");
  ...
  if (insights.production.avgPerDay != null) {
    ...
    lines.push({
      tone,
      text: `${plant.typeLabel} produced ${fmtInt(insights.production.total)} m³ over ${PERIOD_LABEL[period].toLowerCase()}, averaging ${fmtInt(insights.production.avgPerDay)} m³/day${trendStr}.`,
    });
  }
  if (insights.recovery != null) {
    const tone = insights.recovery >= recoveryTarget ? "good" : insights.recovery >= recoveryTarget * 0.85 ? "info" : "warn";
    lines.push({ tone, text: `Overall recovery (production ÷ feed) measured at ${insights.recovery.toFixed(1)}%${insights.recovery < recoveryTarget ? ` — below typical ${plant.typeLabel} target of ${recoveryTargetLabel}` : ""}.` });
  }
  // MBR-only: TMP/fouling commentary
  if (!isRo) {
  ...
  // RO-only: DP / scaling commentary
  if (isRo) {
    const critDp = insights.skids.filter(s => s.dp.hoursCrit > 0);
    if (critDp.length) {
      lines.push({
        tone: "bad",
        text: `${critDp.map(s => s.skid.name).join(", ")} exceeded ΔP > 1.5 bar for ${fmt(critDp.reduce((a, s) => a + s.dp.hoursCrit, 0), 1)} hours — strong scaling/fouling indicator. Plan a CIP and review antiscalant dosing.`,
      });
    }
    const warnDp = insights.skids.filter(s => s.dp.hoursCrit === 0 && s.dp.hoursWarn > 1);
    if (warnDp.length) {
      lines.push({
        tone: "warn",
        text: `${warnDp.map(s => s.skid.name).join(", ")} operated with elevated ΔP (>1.0 bar) for ${fmt(warnDp.reduce((a, s) => a + s.dp.hoursWarn, 0), 1)} hours.`,
      });
    }
    const risingDp = insights.skids.filter(s => s.dp.trend != null && s.dp.trend > 0.05);
    if (risingDp.length) {
      lines.push({
        tone: "warn",
        text: `Stage ΔP is climbing on ${risingDp.map(s => `${s.skid.name} (+${(s.dp.trend! * 1000).toFixed(0)} mbar/day)`).join(", ")} — early-warning of fouling.`,
      });
    }
    const lowReco = insights.skids.filter(s => s.recovery.avg != null && s.recovery.avg > 0 && s.recovery.avg < 30);
    if (lowReco.length) {
      lines.push({
        tone: "warn",
        text: `${lowReco.map(s => `${s.skid.name} (${s.recovery.avg!.toFixed(1)}%)`).join(", ")} averaged a low per-stage recovery — verify membrane condition and feed quality.`,
      });
    }
  }
  ...
  if (flowShares.length >= 2) {
    ...
    if (max - min > 15) {
      ...
      lines.push({ tone: "good", text: `${unit} loading is balanced (max-min ${(max - min).toFixed(1)}% across ${insights.skids.length} ${unit.toLowerCase()}${insights.skids.length === 1 ? "" : "s"}).` });
    }
  }
...
// ── Production analysis ─────────────────────────────────────────────────
function ProductionSection({ insights, dailyRows, plant, overallRecoveryRows }: { insights: Insights; dailyRows: SeriesRow[]; plant: PlantSchema; overallRecoveryRows: SeriesRow[] }) {
  ...
  // For RO plants the SCADA usually exposes a real-time `*_reco` recovery tag
  // that is more accurate than computing production÷feed from totalizers.
  const overallRecoveryAvg = (() => {
    if (!plant.overall.recovery || !overallRecoveryRows.length) return null;
    const xs = overallRecoveryRows.map(r => num(r[plant.overall.recovery!])).filter(v => v != null) as number[];
    if (!xs.length) return null;
    return xs.reduce((a, b) => a + b, 0) / xs.length;
  })();
  const recoveryShown = overallRecoveryAvg ?? insights.recovery;
  const recoverySrc = overallRecoveryAvg != null
    ? "from overall recovery tag"
    : (insights.feed.totalIn ? `Feed in: ${fmtInt(insights.feed.totalIn)} m³` : "feed totalizer not detected");
  const recoveryTarget = plant.type === "ro_reject" ? 50 : (plant.type === "ro" ? 75 : 90);
  ...
  return (
    <SectionCard icon={Activity} title="Production & Recovery Analysis" subtitle="Daily totalizer values and recovery efficiency" accent="emerald">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        ...
        <StatBox
          label="Recovery"
          value={fmtPct(recoveryShown)}
          tone={recoveryShown == null ? "default" : recoveryShown >= recoveryTarget ? "good" : recoveryShown >= recoveryTarget * 0.85 ? "default" : "warn"}
          sub={recoverySrc}
        />
        <StatBox
          label="Trend"
...
function MembraneHealthSection({ insights, opsRows, plant }: { insights: Insights; opsRows: SeriesRow[]; plant: PlantSchema }) {
  // MBR-only section: skip entirely if no skid has a TMP column.
  const hasTmp = plant.skids.some(s => !!s.tmp);
...
function SkidBalancingSection({ insights, plant }: { insights: Insights; plant: PlantSchema }) {
  ...
  return (
    <SectionCard
      icon={Layers}
      title={`${unit} Load Balancing`}
      subtitle={`Average flow per ${unit.toLowerCase()} and contribution to total plant flow`}
      accent="sky"
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5 pt-5 border-t border-slate-100">
        ...
        {hasBw ? (
          ...
          <>
            <StatBox label="Stages with ΔP" value={`${plant.skids.filter(s => s.dp).length}/${plant.skids.length}`} />
            <StatBox label="Stages with Reco" value={`${plant.skids.filter(s => s.recovery).length}/${plant.skids.length}`} />
          </>
        ) : (
          <>
            <StatBox label="Stages w/ Totalizer" value={`${plant.skids.filter(s => s.totalizerDay).length}/${plant.skids.length}`} />
            <StatBox label="Stages w/ Run-hours" value={`${plant.skids.filter(s => s.runHours).length}/${plant.skids.length}`} />
          </>
        )}
        <StatBox label={`${unit}s Active`} value={`${insights.skids.filter(s => s.flow.avg && s.flow.avg > 0).length}/${plant.skids.length}`} />
      </div>
    </SectionCard>
  );
}
...
function DetailedMetricsTable({ insights, plant }: { insights: Insights; plant: PlantSchema }) {
  const unit = plant.unitLabel;
  const isRo = plant.type === "ro" || plant.type === "ro_reject";

  return (
    <SectionCard icon={Layers} title={`Per-${unit} Statistical Summary`} subtitle="Aggregate readings for the reporting period" accent="slate">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-left text-[10px] uppercase tracking-wider text-slate-500 border-b border-slate-200">
              ...
              {isRo ? (
                <>
                  <th className="px-3 py-2 font-bold text-right">In Press Avg</th>
                  <th className="px-3 py-2 font-bold text-right">Out Press Avg</th>
                  <th className="px-3 py-2 font-bold text-right">ΔP Avg</th>
                  <th className="px-3 py-2 font-bold text-right">ΔP Peak</th>
                  <th className="px-3 py-2 font-bold text-right">Recovery Avg</th>
                </>
              ) : (
                <>
              ...
              {!isRo && <th className="px-3 py-2 font-bold text-right">Backwash</th>}
            </tr>
          </thead>
          <tbody>
            {insights.skids.map((si, i) => {
              ...
              return (
                <tr key={si.skid.id} className="border-b border-slate-50">
                  ...
                  {isRo ? (
                    <>
                      <td className="px-3 py-2 text-right text-slate-700">{fmt(si.inP.avg, 2)}</td>
                      <td className="px-3 py-2 text-right text-slate-700">{fmt(si.outP.avg, 2)}</td>
                      <td className={`px-3 py-2 text-right font-semibold ${si.dp.avg != null && si.dp.avg > 1.0 ? "text-amber-700" : "text-slate-700"}`}>{fmt(si.dp.avg, 3)}</td>
                      <td className={`px-3 py-2 text-right font-semibold ${si.dp.max != null && si.dp.max > 1.5 ? "text-rose-700" : si.dp.max != null && si.dp.max > 1.0 ? "text-amber-700" : "text-slate-700"}`}>{fmt(si.dp.max, 3)}</td>
                      <td className={`px-3 py-2 text-right font-semibold ${si.recovery.avg != null && si.recovery.avg > 0 && si.recovery.avg < 30 ? "text-amber-700" : "text-slate-700"}`}>{si.recovery.avg != null ? `${si.recovery.avg.toFixed(1)}%` : "—"}</td>
                    </>
                  ) : (
                    <>
                  ...
                  {!isRo && <td className="px-3 py-2 text-right text-slate-700">{fmtInt(si.bwTotal)}</td>}
                </tr>
              );
            })}
          </tbody>
...
// ── Recommendations ────────────────────────────────────────────────────
function Recommendations({ insights, plant }: { insights: Insights; plant: PlantSchema }) {
  ...
  insights.skids.forEach(si => {
    if (!isRo) {
      ...
      if (si.dp.hoursCrit > 0) {
        recs.push({
          tone: "bad",
          title: `${si.skid.name}: Plan a CIP / antiscalant review`,
          body: `Stage ΔP exceeded 1.5 bar for ${fmt(si.dp.hoursCrit, 1)} hours — strong indicator of scaling or biofouling. Inspect cartridge filters and verify antiscalant dosing rate.`,
        });
      } else if (si.dp.trend != null && si.dp.trend * 1000 > 50) {
        recs.push({
          tone: "warn",
          title: `${si.skid.name}: Rising ΔP trend`,
          body: `Stage ΔP is climbing at ${(si.dp.trend * 1000).toFixed(0)} mbar/day. Schedule a chemical clean before it crosses the 1.5 bar critical threshold.`,
        });
      }
      if (si.recovery.avg != null && si.recovery.avg > 0 && si.recovery.avg < 30) {
        recs.push({
          tone: "warn",
          title: `${si.skid.name}: Low per-stage recovery`,
          body: `Average per-stage recovery of ${si.recovery.avg.toFixed(1)}% is well below design. Verify membrane integrity, normalised permeability, and reject/concentrate valve positions.`,
        });
      }
    }
  });
  ...
  if (flowShares.length >= 2 && Math.max(...flowShares) - Math.min(...flowShares) > 15) {
    ...
    recs.push({
      tone: "warn",
      title: `Rebalance ${plant.unitLabel.toLowerCase()} loading`,
      body: `${heavy.skid.name} is doing ${heavy.share!.toFixed(0)}% of flow vs ${light.skid.name} at ${light.share!.toFixed(0)}%. Adjust valve setpoints or check for blockages on the lighter ${plant.unitLabel.toLowerCase()}.`,
    });
  }

  if (insights.recovery != null && insights.recovery < recoveryTarget) {
    recs.push({
      tone: "warn",
      title: "Investigate low recovery",
      body: `Recovery of ${insights.recovery.toFixed(1)}% is below typical ${plant.typeLabel} target of ${recoveryTarget}%+. ${isRo ? "Inspect concentrate/reject control valve, verify feed pressure, and check for membrane fouling reducing permeate output." : "Verify feed totalizer accuracy and check for reject/leak losses."}`,
    });
  }
...
function OperationalUptimeSection({
  ...
  return (
    <SectionCard
      icon={Clock}
      title={`${plant.unitLabel} Operating Hours & Utilisation`}
...
// ── Stage pressure & ΔP analysis (RO / Reject-RO only) ──────────────────
function StagePressureSection({ insights, opsRows, plant }: { insights: Insights; opsRows: SeriesRow[]; plant: PlantSchema }) {
  const isRo = plant.type === "ro" || plant.type === "ro_reject";
  if (!isRo) return null;

  const hasDp = plant.skids.some(s => !!s.dp);
  const hasInOut = plant.skids.some(s => !!s.inPressure || !!s.outPressure);
  if (!hasDp && !hasInOut) return null;

  const dpChart = hasDp
    ? opsRows.map(r => {
        const o: any = { time: fmtDateTime(r.bucket as string) };
        plant.skids.forEach(s => { if (s.dp) o[s.name] = num(r[s.dp]); });
        return o;
      })
    : [];
  ...
  return (
    <SectionCard
      icon={Gauge}
      title="Stage Pressure & ΔP Analysis"
      subtitle="Per-stage differential pressure trends and inlet/outlet operating pressures"
      accent="rose"
    >
      {hasDp && dpChart.length > 0 && (
        <div className="h-64 mb-5">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={dpChart} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="time" tick={{ fontSize: 10, fill: "#64748b" }} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 11, fill: "#64748b" }} domain={[0, "auto"]} label={{ value: "bar", angle: -90, position: "insideLeft", style: { fontSize: 11, fill: "#64748b" } }} />
              <RTooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <ReferenceLine y={1.0} stroke="#f59e0b" strokeDasharray="3 3" label={{ value: "Warn", position: "right", fill: "#f59e0b", fontSize: 10 }} />
              <ReferenceLine y={1.5} stroke="#ef4444" strokeDasharray="3 3" label={{ value: "Critical", position: "right", fill: "#ef4444", fontSize: 10 }} />
              {plant.skids.filter(s => s.dp).map((s, i) => (
                <Line key={s.id} type="monotone" dataKey={s.name} stroke={SKID_COLORS[i % SKID_COLORS.length]} strokeWidth={2} dot={false} connectNulls />
              ))}
            </LineChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        {insights.skids.map((si, i) => {
          const tone: "good" | "warn" | "bad" =
            si.dp.hoursCrit > 0 ? "bad" : si.dp.hoursWarn > 0 ? "warn" : "good";
          const trendMbar = si.dp.trend != null ? si.dp.trend * 1000 : null;
          return (
            <div key={si.skid.id} className="rounded-xl border border-slate-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between" style={{ background: `${SKID_COLORS[i % SKID_COLORS.length]}10` }}>
                <div className="font-bold text-sm text-slate-900 flex items-center gap-2">
                ...
                {si.skid.dp && (
                  <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                    tone === "bad" ? "bg-rose-100 text-rose-700" : tone === "warn" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"
                  }`}>{tone === "bad" ? "critical" : tone === "warn" ? "warn" : "ok"}</span>
                )}
              </div>
              <div className="p-4 grid grid-cols-2 gap-3 text-xs">
                <div>
                  <div className="text-slate-500">Inlet P avg</div>
                  <div className="text-base font-bold text-slate-900">{fmt(si.inP.avg, 2)} <span className="text-[10px] text-slate-400">bar</span></div>
                </div>
                <div>
                  <div className="text-slate-500">Outlet P avg</div>
                  <div className="text-base font-bold text-slate-900">{fmt(si.outP.avg, 2)} <span className="text-[10px] text-slate-400">bar</span></div>
                </div>
                <div>
                  <div className="text-slate-500">ΔP avg</div>
                  <div className={`text-base font-bold ${si.dp.avg != null && si.dp.avg > 1.0 ? "text-amber-700" : "text-slate-900"}`}>
                    {fmt(si.dp.avg, 3)} <span className="text-[10px] text-slate-400">bar</span>
                  </div>
                </div>
                <div>
                  <div className="text-slate-500">ΔP peak</div>
                  <div className={`text-base font-bold ${si.dp.max != null && si.dp.max > 1.5 ? "text-rose-700" : si.dp.max != null && si.dp.max > 1.0 ? "text-amber-700" : "text-slate-900"}`}>
                    {fmt(si.dp.max, 3)} <span className="text-[10px] text-slate-400">bar</span>
                  </div>
                </div>
                {trendMbar != null && (
                  <div className="col-span-2 pt-2 border-t border-slate-100">
                    <div className="flex justify-between text-[11px]">
                      <span className="text-slate-500">ΔP trend: <strong className={trendMbar > 50 ? "text-rose-700" : trendMbar > 20 ? "text-amber-700" : "text-slate-700"}>{trendMbar > 0 ? "+" : ""}{trendMbar.toFixed(0)} mbar/d</strong></span>
                      {(si.dp.hoursWarn > 0 || si.dp.hoursCrit > 0) && (
                        <span className="text-rose-700">{">"}1.5 bar: <strong>{fmt(si.dp.hoursCrit, 1)}h</strong></span>
                      )}
                    </div>
                  </div>
                )}
...
// ── Recovery breakdown (RO / Reject-RO only) ────────────────────────────
function RecoveryBreakdownSection({ insights, opsRows, plant }: { insights: Insights; opsRows: SeriesRow[]; plant: PlantSchema }) {
  const isRo = plant.type === "ro" || plant.type === "ro_reject";
  if (!isRo) return null;
  const hasOverall = !!plant.overall.recovery;
  const stageRec = plant.skids.filter(s => !!s.recovery);
  if (!hasOverall && stageRec.length === 0) return null;

  const overallSeries = hasOverall
    ? opsRows.map(r => ({ time: fmtDateTime(r.bucket as string), Overall: num(r[plant.overall.recovery!]) }))
    : [];

  const overallAvg = hasOverall && overallSeries.length
    ? overallSeries.map(p => p.Overall).filter(v => v != null).reduce((a, b: any) => a + b, 0) /
      Math.max(1, overallSeries.filter(p => p.Overall != null).length)
    : null;

  const recoveryTarget = plant.type === "ro_reject" ? 50 : 75;

  return (
    <SectionCard
      icon={TrendingUp}
      title="Recovery Breakdown"
      subtitle="Overall plant recovery and per-stage recovery contribution"
      accent="emerald"
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <StatBox
          ...
          unit="%"
          tone={overallAvg == null ? "default" : overallAvg >= recoveryTarget ? "good" : overallAvg >= recoveryTarget * 0.85 ? "default" : "warn"}
          sub={hasOverall ? "from real-time recovery tag" : "no overall recovery tag"}
        />
        <StatBox
          label="Stages with Reco Tag"
          value={`${stageRec.length}/${plant.skids.length}`}
        />
        <StatBox
          label={`Target (${plant.typeLabel})`}
          value={`${recoveryTarget}%+`}
        />
        <StatBox
          label="Stages Below Target"
          value={`${insights.skids.filter(si => si.recovery.avg != null && si.recovery.avg > 0 && si.recovery.avg < recoveryTarget).length}`}
          tone={insights.skids.some(si => si.recovery.avg != null && si.recovery.avg > 0 && si.recovery.avg < recoveryTarget * 0.5) ? "warn" : "default"}
        />
      </div>

      {hasOverall && overallSeries.length > 0 && (
        <div className="h-56 mb-5">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={overallSeries} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="time" tick={{ fontSize: 10, fill: "#64748b" }} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 11, fill: "#64748b" }} domain={[0, 100]} label={{ value: "%", angle: -90, position: "insideLeft", style: { fontSize: 11, fill: "#64748b" } }} />
              <RTooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
              <ReferenceLine y={recoveryTarget} stroke="#10b981" strokeDasharray="3 3" label={{ value: "Target", position: "right", fill: "#10b981", fontSize: 10 }} />
              <Area type="monotone" dataKey="Overall" stroke="#10b981" fill="#d1fae5" strokeWidth={2} connectNulls />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className="space-y-3">
        {insights.skids.map((si, i) => {
          if (si.recovery.avg == null) return null;
          const pct = Math.max(0, Math.min(100, si.recovery.avg));
          const tone = si.recovery.avg >= recoveryTarget ? "emerald" : si.recovery.avg >= 30 ? "amber" : "rose";
          ...
          return (
            <div key={si.skid.id}>
              <div className="flex items-center justify-between text-xs mb-1.5">
                ...
                <span className="text-slate-500">
                  <span className={`font-bold ${toneCls[tone]}`}>{si.recovery.avg.toFixed(1)}%</span> avg ·
                  <span className="ml-1.5">range {fmt(si.recovery.min, 1)}–{fmt(si.recovery.max, 1)}%</span>
                </span>
              </div>
              <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${pct}%`, background: SKID_COLORS[i % SKID_COLORS.length] }}
                />
              </div>
            </div>
          );
