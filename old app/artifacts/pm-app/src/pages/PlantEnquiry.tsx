...
import {
  ...
  Trophy, XCircle, Filter, Mic, Square, Languages, Clipboard, FileEdit,
} from "lucide-react";
...
type EnquiryStatus = "draft" | "new" | "contacted" | "proposal_sent" | "won" | "lost";
...
const STATUS_META: Record<EnquiryStatus, { label: string; tone: string; dot: string; icon: any }> = {
  draft:          { label: "Draft",         tone: "bg-zinc-100 text-zinc-700 border-zinc-200",          dot: "bg-zinc-400",    icon: FileEdit },
...
const STATUS_ORDER: EnquiryStatus[] = ["draft", "new", "contacted", "proposal_sent", "won", "lost"];
...
function SectionCard({
  ...
  return (
    <div className="bg-white border border-slate-200/70 rounded-xl shadow-[0_1px_2px_rgba(15,23,42,0.04),0_4px_16px_-8px_rgba(15,23,42,0.06)] hover:shadow-[0_2px_4px_rgba(15,23,42,0.05),0_8px_24px_-10px_rgba(15,23,42,0.10)] transition-shadow overflow-hidden">
      <div className={cn("flex items-center gap-2 px-4 py-2.5 border-b border-current/5", accent)}>
        <span className="w-6 h-6 rounded-md bg-white/80 ring-1 ring-current/10 flex items-center justify-center shrink-0">
          <Icon className="w-3.5 h-3.5" />
        </span>
        <span className="font-bold text-[11px] uppercase tracking-[0.14em]">{title}</span>
        {badge && (
          <span className="ml-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-white/80 ring-1 ring-current/10 uppercase tracking-wider">
            {badge}
          </span>
        )}
...
const FIELD_LABEL_CLS = "text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-500 flex items-center gap-1.5 mb-1";
const FIELD_INPUT_CLS = "w-full px-2.5 py-1.5 text-[12.5px] text-slate-800 rounded-md border border-slate-200 bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] placeholder:text-slate-400 hover:border-slate-300 focus:outline-none focus:border-[#0a2463] focus:ring-2 focus:ring-[#0a2463]/15 transition";
  ...
  return (
    <label className="block group">
      <span className={FIELD_LABEL_CLS}>
        {Icon && <Icon className="w-3 h-3 text-slate-400 group-focus-within:text-[#0a2463] transition-colors" />} {label}
      ...
      <div className="relative">
        <input
          ...
          className={cn(FIELD_INPUT_CLS, suffix && "pr-12")}
        />
        {suffix && (
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] font-bold text-slate-400 uppercase tracking-wider">{suffix}</span>
        )}
      </div>
    </label>
  );
  ...
  return (
    <label className="block group">
      ...
      <textarea
        ...
        placeholder={placeholder}
        onChange={(e) => onChange(e.target.value)}
        className={cn(FIELD_INPUT_CLS, "resize-none leading-snug")}
      />
    </label>
  );
}
    ...
    <label className="block group">
      ...
      <div className="relative">
        <select
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className={cn(FIELD_INPUT_CLS, "appearance-none pr-7 cursor-pointer")}
        >
...
export default function PlantEnquiry() {
  ...
  const [leadToast, setLeadToast] = useState<{ ok: boolean; msg: string; ts: number } | null>(null);
  const leadConvertedRef = useRef<Set<string>>(new Set());
  ...
  // Auto-save: persist the form to localStorage AND keep a "Draft" entry in the
  // saved-list as soon as the user types something meaningful, so unfinished work
  // is never lost and shows up on the dashboard with a clear Draft badge.
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const hasContent = !!(
        String(form.industry_name ?? "").trim() ||
        String(form.contact_person ?? "").trim() ||
        String(form.mobile_no ?? "").trim() ||
      ...
      if (hasContent) {
        ...
        setList((prev) => {
          ...
          if (savedId && prev.some((e) => e.id === savedId)) {
            next = prev.map((e) => e.id === savedId ? {
              ...e,
              savedAt: now,
              completion,
              form,
              // Keep "draft" until user explicitly saves; otherwise preserve current status.
              status: e.status === "draft" || !e.status ? "draft" : e.status,
            } : e);
          } else {
            const id = savedId || newId();
            if (!savedId) { savedId = id; setCurrentId(id); }
            next = [{ id, savedAt: now, completion, form, status: "draft" }, ...prev];
          }
          saveList(next);
          return next;
        });
    ...
    return () => clearTimeout(t);
  }, [form, autoSaveOn, view, currentId, completion]);
  ...
  const handleSave = (opts: { silent?: boolean } = {}) => {
    ...
    setList((prev) => {
      let next: SavedEnquiry[];
      if (currentId) {
        // Explicit save promotes a Draft to a real "New Lead" entry.
        next = prev.map((e) => e.id === currentId ? {
          ...e,
          savedAt: now,
          completion,
          form,
          status: e.status && e.status !== "draft" ? e.status : "new",
        } : e);
    ...
    // Also push to the Leads pipeline (best-effort, non-blocking).
    // Only convert to a Lead the FIRST time this enquiry is saved, so a
    // re-save / autosave doesn't keep creating duplicate leads in ERPNext.
    const alreadyConverted = currentId ? leadConvertedRef.current.has(currentId) : false;
    if (!alreadyConverted && (form.industry_name || form.contact_person)) {
      const base = import.meta.env.BASE_URL.replace(/\/$/, "");
      const idForLock = savedId;
      fetch(`${base}/api/sales-dashboard/create_lead`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          capacity: form.effluent_capacity,
          requirement: form.present_requirement || form.treatment_required?.join(", "),
        ...
        .then(async (r) => {
          ...
          if (r.ok && j?.success) {
            if (idForLock) leadConvertedRef.current.add(idForLock);
            const leadName = j?.lead?.name ?? "lead";
            console.info("[PlantEnquiry] lead created:", leadName);
            if (!opts.silent) {
              setLeadToast({ ok: true, msg: `Saved & converted to Lead ${leadName}`, ts: Date.now() });
            }
          } else {
            console.warn("[PlantEnquiry] create_lead rejected:", j?.message ?? r.status);
            if (!opts.silent) {
              setLeadToast({ ok: false, msg: `Lead conversion failed: ${j?.message ?? r.statusText}`, ts: Date.now() });
            }
          }
        })
        .catch((e) => {
          console.warn("[PlantEnquiry] create_lead failed:", e);
          if (!opts.silent) {
            setLeadToast({ ok: false, msg: `Lead conversion failed: ${String(e?.message ?? e)}`, ts: Date.now() });
          }
        });
    }
  };
  ...
  // Auto-dismiss lead-conversion toast
  useEffect(() => {
    if (!leadToast) return;
  ...
  return (
    <Layout>
      <div className="p-3 lg:p-4 h-[calc(100vh-0px)] flex flex-col gap-3 overflow-hidden bg-gradient-to-b from-slate-50/50 to-transparent">
        {/* Compact header */}
        <div className="relative bg-white border border-slate-200/70 rounded-lg px-3 py-2 flex items-center gap-3 shrink-0 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_4px_12px_-6px_rgba(15,23,42,0.06)] overflow-hidden">
          <span aria-hidden className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-[#0a2463] via-[#0ea5e9] to-[#0a2463]" />
          <button onClick={handleBackToDashboard} title="Back to dashboard" className="w-7 h-7 rounded-md bg-slate-100 hover:bg-slate-200 flex items-center justify-center shrink-0 transition ml-1">
            <ChevronDown className="w-4 h-4 text-slate-700 rotate-90" />
          </button>
          <div className="flex-1 min-w-0">
            <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 flex items-center gap-1.5">
              <span className="text-[#0a2463]">WTT International</span>
              <span className="text-slate-300">·</span>
              <span>{currentId ? "Editing Enquiry" : "New Enquiry"}</span>
            </div>
            <h1 className="text-[14px] font-bold tracking-tight leading-tight text-slate-900 truncate">Questionnaire for New Plant</h1>
          </div>
          <div className="hidden md:flex items-center gap-2 px-2.5 py-1 rounded-md bg-slate-50/80 ring-1 ring-slate-200/70">
            <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">Completion</span>
            <div className="w-24 h-1.5 rounded-full bg-slate-200 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#0a2463] to-[#0ea5e9] transition-all rounded-full" style={{ width: `${completion}%` }} />
            </div>
            <span className="text-[11px] font-bold tabular-nums text-[#0a2463]">{completion}%</span>
          </div>
        ...
        {/* Lead-conversion toast */}
        {leadToast && (
          <div className={cn(
            "rounded-md px-3 py-2 flex items-center gap-3 shrink-0 shadow-sm border",
            leadToast.ok
            ...
            <div className={cn(
              ...
              leadToast.ok ? "bg-emerald-500" : "bg-rose-500",
            )}>
              {leadToast.ok ? <BadgeCheck className="w-4 h-4 text-white" /> : <X className="w-4 h-4 text-white" />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[11px] font-bold leading-tight">
                {leadToast.ok ? "Lead created in CRM" : "Could not create lead"}
              </div>
              <div className="text-[10px] opacity-80 leading-tight mt-0.5 truncate">{leadToast.msg}</div>
            </div>
            <button onClick={() => setLeadToast(null)} title="Dismiss" className="p-1 rounded hover:bg-white/40">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        )}
