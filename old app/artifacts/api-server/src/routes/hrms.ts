...
router.get("/users/mention", async (req, res) => {
  try {
    ...
    const max = Math.max(1, Math.min(parseInt(limit, 10) || 20, 1000));
    ...
    const filtered = users
      ...
      .map(u => ({
        ...
        department: u.email,
        avatar: u.user_image
          ? (u.user_image.startsWith("http")
              ? u.user_image
              : `${ERPNEXT_URL}${u.user_image.split("/").map((p: string) => p ? encodeURIComponent(p) : "").join("/")}`)
          : null,
      }));
    res.json(filtered);
  } catch (e) {
...
router.get("/employee-dashboard", async (req, res) => {
  try {
    ...
    // Checkins are tolerated to fail (some ERPNext setups have permission rules
    // that inject filters on columns the Employee Checkin doctype doesn't have);
    // in that case we just treat checkins as empty so the dashboard still loads.
    const [attendanceRecords, checkins] = await Promise.all([
      ...
      employeeId
        ? fetchErpNextCheckins({ employee: employeeId, from_date: today, to_date: today }).catch(err => {
            console.warn(`[employee-dashboard] checkins fetch failed: ${err?.message || err}`);
            return [] as any[];
          })
        : Promise.resolve([]),
    ]);
